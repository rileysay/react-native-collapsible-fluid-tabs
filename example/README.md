...
The Lab starts with a collapsing header and **Scroll behind notch** enabled. On web, **Pinned header** is on by default, including after **Reset to defaults**. The docs phone preview supplies a simulated top safe area, keeping the pinned toolbar below the status bar. Native starts with the pinned header off. With no pinned header and **Scroll behind notch** on, `topInset={0}` lets the initial safe-area padding scroll away with the header, and the tab bar can reach the cutout area; the bottom toolbar remains available. Turn **Scroll behind notch** off to restore the inset. Standalone web uses the browser's safe-area values, and native uses the device's insets.
